...
export const isAuthorized = () => expressJwt(({ secret: publicKey, algorithms: jwtAlgorithms }) as any)
...
export const verify = (token: string) => {
  ...
  try {
    return jwt.verify(token, publicKey, { algorithms: jwtAlgorithms } as any) !== undefined
  } catch {
    return false
  }
}
...
export const updateAuthenticatedUsers = () => (req: Request, res: Response, next: NextFunction) => {
  ...
  if (token) {
    jwt.verify(token, publicKey, { algorithms: jwtAlgorithms } as any, (err: Error | null, decoded: any) => {
      if (err === null) {
